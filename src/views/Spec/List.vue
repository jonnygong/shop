<template>
  <section>
    ...
    <el-table :data="list"
      ...
      <el-table-column prop="title" label="自定义规格名字" width="180">
      </el-table-column>
      <el-table-column prop="is_use" label="是否使用商品原有价格" width="180">
        <template slot-scope="scope">
          {{ is_use[scope.row.is_use] }}
        </template>
      </el-table-column>
      <!-- 普通列表显示 -->
      <el-table-column
...
//        },
    ...
    methods: {
      ...
      async getListData () {
        ...
        const res = await this.$http.post(`specList`, params)
      ...
      handleDel (index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          const res = await this.$http.post(`specStatus`, params)
          if (res === null) return
          this.$message({
            message: '状态修改成功',
            type: 'success'
      ...
      // 显示新增界面
      handleAdd () {
        console.log(`${this.$route.path}/${this.$route.params.goods_id}/add`)

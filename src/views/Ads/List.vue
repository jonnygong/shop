<template>
  <section>
    ...
    <el-table :data="list"
      ...
      <el-table-column prop="place_id" label="广告位" width="120">
        <template slot-scope="scope">
          {{ place_id[scope.row.place_id] }}
        </template>
      </el-table-column>
      <!-- 普通列表显示 -->
      <el-table-column
  ...
  export default {
    ...
    methods: {
      ...
      async getListData () {
        ...
        const res = await this.$http.post(`adsList`, params)
      ...
      handleDel (index, row) {
        this.$confirm('确认删除该记录吗?', '提示', {
          ...
          const res = await this.$http.post(`adsStatus`, params)
          if (res === null) return
          this.$message({
            message: '状态修改成功',
            type: 'success'
